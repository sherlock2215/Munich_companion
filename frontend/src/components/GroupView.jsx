...
import ChatRoom from './ChatRoom'; // Wichtig: Muss importiert sein
import { ArrowLeft, Users, MessageCircle, Plus, CheckCircle } from 'lucide-react'; // Send entfernt, da es nur im ChatRoom ist
...
const GroupView = ({ place, user, onBack }) => {
    ...
    const loadGroups = async () => {
        ...
        try {
            ...
            // Sicherstellen, dass data ein Array ist oder auf leeres Array gesetzt wird
            setGroups(data || []);
        } catch (err) {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            // Nach Erstellen zurück zur Liste und NEU LADEN, um den Blackscreen zu verhindern
            setView("list");
            await loadGroups();
        } catch (err) {
    ...
    return (
        <div style={{height: '100%', display: 'flex', flexDirection: 'column', color: '#1e293b'}}>
            ...
            <div style={{flex: 1, overflowY: 'auto', padding: '15px', background: '#f8fafc'}}>
                ...
                {groups.map(group => {
                    // FEHLERBEHEBUNG: Prüfen ob group.members existiert und ein Array ist
